export default {
    ...
    methods:
        {
            async validateAssignment_id(assignment_id)
            {

                if(!(assignment_id.length === 24))
                {
                    this.isAssignment_idValid = false;
                    this.errorAlert = "Assignment id is invalid";
                }
                else
                {
            ...
            async fetchCalendar()
            {
                ...
                let requestBody = {assignment_id: assignment_id,};
                let requestUrl = "/api/calendarAssignmentRead";
                ...
                if ((data) && (!data.errors))
                {
                    ...
                    this.isAssignment_idValid = true;
            ...
            populateCalendar()
            {
                //get shared assignment from assignment_id
                let assignment = this.calendar.assignments.find(assignment => assignment._id === this.assignment_id);
                ...
                let currentMondayStartDate = new Date(assignment.date);
    ...
    async mounted()
    {
        await this.validateAssignment_id(this.$route.params.assignment_id);
    },
